...
const getProducts = async (req, res) => {
  ...
  try {
    ...
    if (diet) {
      products = products.filter((s) => s.diets.find((d) => d.name === diet));
      if (!products.length) {
        throw new Error("No hay productos asociados a esa dieta");
      }
    }
    if (price) {
      products = products = products.filter((s) => s.price < price);
      if (!products.length) {
...
const postProduct = async (req, res) => {
  let { name, price, realValue, description, image, diets, stock, sellerId } =
    req.body;
...
const restoreProduct = async (req, res) => {
  ...
  try {
    ...
    const restoredProd = await Product.findOne({
      where: {
        id: id,
      },
    });
    res.status(200).send(restoredProd);
  } catch (error) {
    res.status(400).send("Hubo un problema recuperando el producto");
  }
};
...
const disableProduct = async (req, res) => {
  ...
  try {
    ...
    const disableProduct = await Product.findOne({
      where: {
        id: id,
      },
    });
    res.status(200).send(disableProduct);
  } catch (error) {
    res.status(400).send("Hubo un problema al eliminar el producto");
  }
};
